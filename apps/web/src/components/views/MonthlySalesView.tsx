...
export default function MonthlySalesView({ onJumpToDate }: MonthlySalesViewProps) {
  ...
  // Dedicated backend monthly pre-aggregated report query:
  const { data: report, isLoading } = useQuery({
    queryKey: ['web-monthly-report', cursor.year, cursor.month],
    queryFn: () =>
      apiFetch<MonthlyReportResponse>(
        `/reports/monthly?startDate=${monthStart}&endDate=${monthEnd}&startUtc=${startUtc}&endUtc=${endUtc}`,
  ...
  // Fast day rows construction using Map lookups over pre-aggregated data:
  ...
  const cafeRows = useMemo<CafeDayRow[]>(() => {
    ...
    for (const c of cafeTotals) {
      ...
      if (c.method === 'Cash' || c.method === 'Card' || c.method === 'Credit') {
        entry[c.method as 'Cash' | 'Card' | 'Credit'] += c.total;
      }
      entry.total += c.total;
  ...
  const expensesRows = useMemo<ExpensesDayRow[]>(() => {
    ...
    for (const e of expenseTotals) {
      ...
      if (e.method === 'Cash' || e.method === 'Card') {
        entry[e.method as 'Cash' | 'Card'] += e.total;
      }
      entry.total += e.total;
