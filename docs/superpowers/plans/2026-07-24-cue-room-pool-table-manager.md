...
- **Soft deletes:** every syncable table has `updatedAt` and `deletedAt` columns, both locally (Dexie) and remotely (Postgres) — hard deletes are never used, because sync needs tombstones to propagate deletions.
