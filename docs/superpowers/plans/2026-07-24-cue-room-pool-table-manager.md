...
Build the date-stepper header (prev/next/Today using `dateStrOf`/`parseDate` from `@cue-room/shared`), the Total/Card/Cash/Credit summary strip (computed with `useMemo` from `sessions`), and per-category groups of shadcn `Card`s — one card per resource, an inline row per session (time-based: `Input type="time"` × 2, `Input type="number"` for amount, shadcn `Select` for method, shadcn `Select` for customer gated on `useCustomers().customers.length`, delete `Button`), an "Add session" `Button` calling `addSession`, and a subtotal computed via `useMemo`. Do not use `react-hook-form` for these rows — each row commits independently on blur/change (call `updateSession(id, patch)` directly `onChange`, matching the original spec's per-field auto-save behavior), which doesn't fit react-hook-form's submit-a-form model. Instead: validate each patch with `SessionSchema.partial()` client-side before calling `updateSession`, and show the error inline via `field.tsx`'s `FieldError` (pass it the zod error's `.issues` mapped to `{message}` objects) if invalid, e.g. Credit method with no customer selected.
